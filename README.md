...
| `replace` | Delete the measurement (all time range), then write (default for full). With `if_exists: append`: same behavior (delete + write) |
...
Auto-added columns (`_ingested_at`, `mkpipe_id`) are excluded from fields. Both column names are configurable via `settings.ingested_at_column` and `settings.ingestion_id_column`.
...
| `if_exists` | string | — | `replace` (delete+write) or `append` (delete measurement+write). Inherits from settings |
